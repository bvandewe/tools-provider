"""API proxy controller for forwarding requests to tools-provider.
...
Mounted at /tools sub-app, the ApiController handles paths like:
    /tools/api/files/upload -> tools-provider /api/files/upload
    /tools/api/files/{filename} -> tools-provider /api/files/{filename}
"""
...
class ApiController(ControllerBase):
    """Controller that proxies /tools/api/* requests to tools-provider.
    ...
    Mounted at /tools sub-app, this controller handles /api/* paths.
    """
    ...
    @get(
        "/{path:path}",
        summary="Proxy GET request to tools-provider",
        description="Forwards GET requests to tools-provider /api/* endpoints",
    )
    async def proxy_get(
        self,
        path: str,
        ...
        """Proxy generic GET requests to tools-provider."""
        return await self._proxy_request(
            request=request,
            path=f"/api/{path}",
            method="GET",
    ...
    @post(
        "/{path:path}",
        summary="Proxy POST request to tools-provider",
        description="Forwards POST requests to tools-provider /api/* endpoints",
    )
    async def proxy_post(
        ...
        """Proxy generic POST requests to tools-provider."""
    ...
    @put(
        "/{path:path}",
        summary="Proxy PUT request to tools-provider",
        description="Forwards PUT requests to tools-provider /api/* endpoints",
    )
    async def proxy_put(
        self,
        path: str,
        ...
        """Proxy PUT requests to tools-provider."""
        body = await request.body()
        content_type = request.headers.get("Content-Type")
    ...
    @delete(
        "/{path:path}",
        summary="Proxy DELETE request to tools-provider",
        description="Forwards DELETE requests to tools-provider /api/* endpoints",
    )
    async def proxy_delete(
        ...
        """Proxy DELETE requests to tools-provider."""
        return await self._proxy_request(
            request=request,
            path=f"/api/{path}",
            method="DELETE",
