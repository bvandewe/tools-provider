...
import { startSessionMonitoring, stopSessionMonitoring, enableProtection, disableProtection, notifyTokenExpired, hasPendingExpiration } from './core/session-manager.js';
import {
    initDraftManager,
    stopDraftManager,
    setConversation as setDraftConversation,
    saveCurrentDraft,
    restoreDraft,
    clearCurrentDraft,
    hasDraft,
    clearAllStoredDrafts,
} from './core/draft-manager.js';
...
export class ChatApp {
     ...
     * This is the cleanup callback - draft saving is handled by beforeRedirect callback
     */
    handleSessionExpired() {
        ...
        // Stop session monitoring (if not already stopped)
    ...
    /**
     * Handle before redirect - save draft before session redirect
     * Called by session manager before redirecting to login
     */
     ...
     * Protected when: streaming OR has unsent draft in input
     */
    ...
    async checkAuth() {
        try {
            ...
            // Initialize draft manager with message input
            initDraftManager(this.messageInput);

            // Restore any saved draft from previous session
            ...
            // Start session monitoring with callbacks for expiration and before-redirect
            await startSessionMonitoring(
                () => this.handleSessionExpired(),
                () => this.handleBeforeRedirect()
            );

            // Set up input change listener to update protection state
            this.messageInput?.addEventListener('input', () => this.updateSessionProtection());
        } catch (error) {
    ...
    async loadConversation(conversationId) {
        try {
            ...
            if (savedDraft && this.messageInput) {
    ...
    async handleSubmit(e) {
        ...
        // Set streaming state and enable session protection
        ...
        // Reset streaming state and disable protection
        this.isStreaming = false;
        this.userHasScrolled = false;
        api.clearRequestState();
        this.updateSendButton(false);
        this.updateSessionProtection(); // Will disable if no draft
        ...
        // Check if session expired while streaming
    ...
    async sendMessage(message, thinkingElement) {
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                ...
                // Session expired during streaming - notify session manager
                console.log('[ChatApp] Session expired during streaming');
                thinkingElement.setAttribute('content', assistantContent || '_Session expired during response_');
                thinkingElement.setAttribute('status', 'complete');
                this.setStatus('disconnected', 'Session expired');
                // Notify session manager - it will show the modal since we're in protected mode
                notifyTokenExpired();
            } else {
        ...
        switch (eventType) {
            ...
            case 'error':
                ...
                if (errorCode === 'unauthorized' || errorCode === 'session_expired') {
                    console.log('[ChatApp] Auth error in stream:', errorCode);
    ...
    logout() {
        // Clear drafts on explicit logout (user doesn't expect them back)
        clearAllStoredDrafts();
        stopDraftManager();
