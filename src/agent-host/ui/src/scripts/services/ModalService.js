 ...
 *
 * Manages Bootstrap modals for the application, including:
 * - Rename/Delete/Share modals for conversations
 * - Tools modal with search and refresh
 * - Health check modal
 * - Tool details modal with tabbed interface
 * - Conversation info modal with statistics
 * - Permissions modal
 * - Toast notifications
 *
...
/**
 * ModalService manages all Bootstrap modal interactions
 */
export class ModalService {
    ...
    constructor() {
        ...
        /** @type {Object} Modal state for callbacks */
        this._modalState = {
            renameCallback: null,
            deleteCallback: null,
    ...
    init() {
        ...
        this._initRenameModal();
        this._initDeleteModal();
        this._initDeleteAllUnpinnedModal();
    ...
    /**
     * Initialize rename modal event handlers
     * @private
     */
    _initRenameModal() {
        ...
        if (!renameModal || !renameInput || !renameConfirmBtn) return;
        ...
        renameModal.addEventListener('hidden.bs.modal', () => {
            this._modalState.renameCallback = null;
            this._modalState.currentConversationId = null;
            this._modalState.currentTitle = null;
        });

        // Handle confirm button
        renameConfirmBtn.addEventListener('click', () => {
            const newTitle = renameInput.value.trim();
            if (newTitle && this._modalState.renameCallback) {
                this._modalState.renameCallback(this._modalState.currentConversationId, newTitle);
            }
    ...
    /**
     * Initialize delete modal event handlers
     * @private
     */
    _initDeleteModal() {
        ...
        deleteModal.addEventListener('hidden.bs.modal', () => {
            this._modalState.deleteCallback = null;
            this._modalState.currentConversationId = null;
        });

        // Handle confirm button
        deleteConfirmBtn.addEventListener('click', () => {
            if (this._modalState.deleteCallback && this._modalState.currentConversationId) {
                this._modalState.deleteCallback(this._modalState.currentConversationId);
            }
    ...
    /**
     * Initialize delete all unpinned modal event handlers
     * @private
     */
    _initDeleteAllUnpinnedModal() {
        ...
        if (!deleteAllUnpinnedModal || !deleteAllUnpinnedConfirmBtn) return;

        // Clear state when modal hides
        deleteAllUnpinnedModal.addEventListener('hidden.bs.modal', () => {
            this._modalState.deleteAllUnpinnedCallback = null;
        });

        // Handle confirm button
        deleteAllUnpinnedConfirmBtn.addEventListener('click', () => {
            if (this._modalState.deleteAllUnpinnedCallback) {
                this._modalState.deleteAllUnpinnedCallback();
            }
     ...
     */
    showRenameModal(conversationId, currentTitle, onConfirm) {
        const renameModal = document.getElementById('rename-modal');
        const renameInput = document.getElementById('rename-input');

        if (!renameModal || !renameInput) {
            console.error('[ModalService] Rename modal elements not found');
            // Fallback to prompt
        ...
        // Store state
        this._modalState.currentConversationId = conversationId;
        this._modalState.currentTitle = currentTitle;
        this._modalState.renameCallback = onConfirm;
     ...
     */
    showDeleteModal(conversationId, title, onConfirm) {
        const deleteModal = document.getElementById('delete-modal');
        const deleteTitle = document.getElementById('delete-conversation-title');

        if (!deleteModal) {
            console.error('[ModalService] Delete modal not found');
            // Fallback to confirm
        ...
        // Store state
        this._modalState.currentConversationId = conversationId;
        this._modalState.deleteCallback = onConfirm;
     ...
     */
    showDeleteAllUnpinnedModal(count, onConfirm) {
        const deleteAllUnpinnedModal = document.getElementById('delete-all-unpinned-modal');
        ...
        if (!deleteAllUnpinnedModal) {
            console.error('[ModalService] Delete all unpinned modal not found');
            // Fallback to confirm
        ...
        // Store callback
        this._modalState.deleteAllUnpinnedCallback = onConfirm;
     ...
     */
    showConversationInfoModal(conversation) {
        ...
        if (!modal || !contentEl) {
            console.error('[ModalService] Conversation info modal elements not found');
            return;
        }
        ...
        if (toolsUsed.size > 0) {
            const toolBadges = Array.from(toolsUsed)
                .map(tool => `<span class="badge bg-info bg-opacity-10 text-info me-1 mb-1">${this._escapeHtml(this._formatToolName(tool))}</span>`)
                .join('');
            toolsHtml = `
                <div class="mt-3">
        ...
        contentEl.innerHTML = `
            <div class="conversation-info">
                <h6 class="border-bottom pb-2 mb-3">${this._escapeHtml(conversation.title || 'Untitled Conversation')}</h6>
                ...
                <div class="details-section">
                    <div class="d-flex justify-content-between text-muted small mb-2">
                        ...
                        <span>${this._formatBytes(totalBytes)}</span>
                    </div>
                    <div class="d-flex justify-content-between text-muted small mb-2">
                ...
                <div class="mt-3 pt-3 border-top">
                    <small class="text-muted">
                        <i class="bi bi-hash me-1"></i>ID: <code>${this._escapeHtml(conversation.id)}</code>
                    </small>
                </div>
            </div>
        `;
    ...
    showShareModal(conversation) {
        ...
        if (!modal || !contentEl) {
            console.error('[ModalService] Share modal elements not found');
            return;
        }
        ...
        contentEl.innerHTML = `
            <div class="share-content">
                ...
                <h6 class="text-secondary mb-3">${this._escapeHtml(conversation.title || 'Untitled Conversation')}</h6>
            ...
            btn.addEventListener('click', async () => {
                ...
                try {
                    if (format === 'json') {
                        ...
                        this.showToast('Conversation copied as JSON', 'success');
                    } else if (format === 'text') {
                        await navigator.clipboard.writeText(textExport);
                        this.showToast('Conversation copied as text', 'success');
                        ...
                        this.showToast('Conversation downloaded', 'success');
                    }

                    // Close modal after action
                    bootstrap.Modal.getInstance(modal)?.hide();
                } catch (error) {
                    console.error('[ModalService] Share action failed:', error);
                    this.showToast('Failed to share conversation', 'error');
                }
            });
        });
     ...
     */
    showToolsModal(tools, onRefresh = null) {
        ...
        if (!toolsList || !toolsModal) {
            console.error('[ModalService] Tools modal elements not found');
            return;
        }
        ...
        // Render tools list
        this._renderToolsList(toolsList, currentTools);
        ...
        if (searchInput) {
            ...
            searchInput.oninput = () => {
                ...
                if (!query) {
                    ...
                    this._renderToolsList(toolsList, filtered);
     ...
     */
    showToolDetailsModal(toolCalls, toolResults, options = {}) {
        ...
        if (!toolDetailsModal || !toolCallContent) {
            console.error('[ModalService] Tool details modal elements not found');
            return;
        }
        ...
        toolCallContent.innerHTML = this._renderToolDetails(toolExecutions);
        ...
        if (isAdmin && sourceInfoTabItem && sourceInfoTab && sourceInfoContent) {
            ...
            const loadSourceInfo = async () => {
                ...
                try {
                    ...
                    sourceInfoContent.innerHTML = this._renderSourceInfo(sourceInfos);
                } catch (err) {
                    sourceInfoContent.innerHTML = `
                        <div class="alert alert-danger" role="alert">
                            <i class="bi bi-exclamation-triangle me-2"></i>
                            Failed to load source information: ${this._escapeHtml(err.message)}
                        </div>
                    `;
                }
            };
    ...
    async showHealthModal(healthCheckFn) {
        ...
        // Run health check
        await this._runHealthCheck(healthCheckFn, healthContent);
    ...
    showPermissionsModal(scopes) {
        ...
        if (!modal || !contentEl) {
            console.error('[ModalService] Permissions modal elements not found');
            return;
        }
                ...
                .map(
                    ...
                    <div class="mb-3">
                        <h6 class="text-muted text-uppercase small mb-2">
                            <i class="bi bi-folder me-1"></i>${this._escapeHtml(group)}
                        </h6>
                        <div class="d-flex flex-wrap gap-2">
                            ${groupScopes.map(s => `<span class="badge bg-info text-dark">${this._escapeHtml(s)}</span>`).join('')}
                        </div>
                    </div>
                `
                )
     ...
     * @private
     */
    _renderToolsList(container, tools) {
        container.innerHTML = '';
        ...
        tools.forEach(tool => {
            ...
            const displayName = this._formatToolName(tool.name);
            ...
            item.innerHTML = `
                <div class="tool-header">
                    <span>🔧</span>
                    <span class="tool-name">${this._escapeHtml(displayName)}</span>
                </div>
                ...
                <div class="tool-params">
                    ${(tool.parameters || [])
                        .map(
                            p => `
                        <span class="param">
                            <span class="param-name">${this._escapeHtml(p.name)}</span>
                            <span class="param-type">(${this._escapeHtml(p.type)})</span>
                        </span>
                    `
                        )
                        .join('')}
     ...
     * @private
     */
    _renderToolDetails(toolExecutions) {
                ...
                if (exec.hasResult) {
                    if (exec.success && exec.result !== undefined) {
                        try {
                            ...
                            resultContent = `
                                <div class="mt-3">
                                    ...
                                    <pre class="bg-dark text-light p-3 rounded small" style="max-height: 300px; overflow: auto;"><code>${this._escapeHtml(formatted)}</code></pre>
                                </div>
                            `;
                        } catch (e) {
                        ...
                        if (isInsufficientScope) {
                            ...
                            let missingScopes = [];
                            const scopeMatch =
                                exec.error.match(/missing[:\s]+\[?([^\]]+)\]?/i) || exec.error.match(/required[:\s]+\[?([^\]]+)\]?/i) || exec.error.match(/scopes?[:\s]+\[?([^\]]+)\]?/i);
                            if (scopeMatch) {
                                ...
                                <div class="mt-3">
                                    <div class="alert alert-warning mb-0">
                                        ...
                                        ${
                                            ...
                                            <div class="mb-2">
                                                ${missingScopes.map(s => `<span class="badge bg-secondary me-1">${this._escapeHtml(s)}</span>`).join('')}
                                            </div>
                                        `
                                                : ''
                                        }
                            ...
                            resultContent = `
                                <div class="mt-3">
                                    ...
                                    <pre class="bg-danger bg-opacity-10 text-danger p-3 rounded small">${this._escapeHtml(exec.error)}</pre>
                                </div>
                            `;
                        }
                    }
                    ...
                    try {
                        ...
                        argsContent = `
                            <div class="mt-3">
                                ...
                                <pre class="bg-secondary bg-opacity-10 p-3 rounded small" style="max-height: 150px; overflow: auto;"><code>${this._escapeHtml(formatted)}</code></pre>
                            </div>
                        `;
                    } catch (e) {
                    ...
                    <div class="tool-execution-card ${index > 0 ? 'mt-4 pt-4 border-top' : ''}">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <div class="d-flex align-items-center">
                                ...
                                <h6 class="mb-0 fw-bold">${this._escapeHtml(exec.toolName)}</h6>
                            </div>
                            <div>
                                ${statusBadge}
                                ${executionTime}
                        ...
                        ${exec.callId ? `<p class="text-muted small mb-0"><i class="bi bi-hash me-1"></i>Call ID: <code>${this._escapeHtml(exec.callId)}</code></p>` : ''}
                        ${argsContent}
                        ${resultContent}
                    </div>
                `;
                    ...
                    return `
                        <div class="source-info-card ${index > 0 ? 'mt-4 pt-4 border-top' : ''}">
                            <h6 class="fw-bold text-danger">
                                <i class="bi bi-exclamation-triangle me-2"></i>
                                ${this._escapeHtml(item.toolName)}
                            </h6>
                            <p class="text-muted small">Failed to load source info: ${this._escapeHtml(item.error)}</p>
                        </div>
                    `;
                }
                    ...
                    <div class="source-info-card ${index > 0 ? 'mt-4 pt-4 border-top' : ''}">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <div class="d-flex align-items-center">
                                ...
                                <h6 class="mb-0 fw-bold">${this._escapeHtml(item.toolName)}</h6>
                            </div>
                            <div>
                                ${statusBadge}
                                ${enabledBadge}
                        ...
                        <table class="table table-sm table-borderless mb-0">
                            <tbody>
                                <tr>
                                    ...
                                    <td><strong>${this._escapeHtml(info.source_name)}</strong></td>
                                </tr>
                                <tr>
                                    <td class="text-muted"><i class="bi bi-link-45deg me-2"></i>Source URL</td>
                                    <td>
                                        <a href="${this._escapeHtml(info.source_url)}" target="_blank" rel="noopener noreferrer" class="text-decoration-none">
                                            ${this._escapeHtml(info.source_url)}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="${this._escapeHtml(info.openapi_url)}" target="_blank" rel="noopener noreferrer" class="text-decoration-none">
                                            ${this._escapeHtml(info.openapi_url)}
                                            <i class="bi bi-box-arrow-up-right ms-1 small"></i>
                                        </a>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td><code>${this._escapeHtml(info.source_type)}</code></td>
                                </tr>
                                ${
                                ...
                                <tr>
                                    ...
                                    <td><code>${this._escapeHtml(info.default_audience)}</code></td>
                                </tr>
                                `
                                        : ''
                                }
                                ...
                                <tr>
                                    ...
                                    <td>${this._formatDateTime(info.last_sync_at)}</td>
                                </tr>
                                `
                                        : ''
                                }
     ...
     * @private
     */
    async _runHealthCheck(healthCheckFn, container) {
        try {
            const health = await healthCheckFn();
            this._renderHealthStatus(container, health);
        } catch (error) {
            container.innerHTML = `
                <div class="alert alert-danger" role="alert">
                    <i class="bi bi-exclamation-triangle me-2"></i>
                    <strong>Health check failed:</strong> ${this._escapeHtml(error.message)}
                </div>
            `;
        }
    }
     ...
     * @private
     */
    _renderHealthStatus(container, health) {
        const statusIcon = {
            ...
            .map(comp => {
                ...
                if (comp.details) {
                    const detailItems = Object.entries(comp.details)
                        ...
                        .map(([key, value]) => `<span class="badge bg-light text-dark me-1">${this._escapeHtml(key)}: ${this._escapeHtml(String(value))}</span>`)
                        .join('');
                    if (detailItems) {
                    ...
                    <div class="health-component ${comp.status}">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                ...
                                <span class="component-name">${this._escapeHtml(comp.name)}</span>
                            </div>
                            <div>
                        ...
                        <p class="text-muted small mb-0 mt-1">${this._escapeHtml(comp.message)}</p>
                        ${detailsHtml}
                    </div>
                `;
            })
            ...
            <div class="health-overview mb-3">
                <div class="d-flex align-items-center justify-content-between p-3 bg-${overallStatusClass} bg-opacity-10 rounded">
                    <div class="d-flex align-items-center">
                        ...
                        <span class="ms-2 fw-bold">${this._escapeHtml(health.message)}</span>
                    </div>
...
export default {
    initModals: () => modalService.init(),
    showRenameModal: (...args) => modalService.showRenameModal(...args),
    showDeleteModal: (...args) => modalService.showDeleteModal(...args),
    showDeleteAllUnpinnedModal: (...args) => modalService.showDeleteAllUnpinnedModal(...args),
    showToolsModal: (...args) => modalService.showToolsModal(...args),
    showToast: (...args) => modalService.showToast(...args),
    showHealthModal: (...args) => modalService.showHealthModal(...args),
    showToolDetailsModal: (...args) => modalService.showToolDetailsModal(...args),
    showConversationInfoModal: (...args) => modalService.showConversationInfoModal(...args),
    showShareModal: (...args) => modalService.showShareModal(...args),
    showPermissionsModal: (...args) => modalService.showPermissionsModal(...args),
};
