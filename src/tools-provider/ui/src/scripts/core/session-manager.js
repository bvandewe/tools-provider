...
function recordActivity() {
    ...
    // If warning was shown and user becomes active, they need to click Continue
    // Don't auto-dismiss the warning
}
...
async function extendSession() {
    ...
    if (success) {
        ...
        try {
            const { showToast } = await import('../components/toast-notification.js');
...
function initializeSessionIframe() {
    ...
    // Note: We don't poll /api/auth/me continuously anymore.
    // The iframe will notify us via postMessage when the Keycloak session changes.
    // This prevents aggressive polling that can cause reload loops.
