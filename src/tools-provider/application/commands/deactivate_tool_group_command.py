"""Deactivate tool group command with handler."""
...
from observability import tool_group_processing_time, tool_groups_deactivated
...
@dataclass
class DeactivateToolGroupCommand(Command[OperationResult[ToolGroupDto]]):
    """Command to deactivate a tool group."""

    group_id: str
    """ID of the group to deactivate."""
...
class DeactivateToolGroupCommandHandler(
    CommandHandlerBase,
    CommandHandler[DeactivateToolGroupCommand, OperationResult[ToolGroupDto]],
):
    """Handler for deactivating tool groups."""
    ...
    async def handle_async(self, request: DeactivateToolGroupCommand) -> OperationResult[ToolGroupDto]:
        """Handle the deactivate tool group command."""
        command = request
        ...
        add_span_attributes(
            {
                ...
                "tool_group.has_reason": command.reason is not None,
        ...
        # Create custom span for deactivate logic
        with tracer.start_as_current_span("deactivate_tool_group_entity") as span:
            ...
            span.set_attribute("tool_group.deactivated_by", deactivated_by or "unknown")

            # Deactivate
            deactivated = tool_group.deactivate(
                deactivated_by=deactivated_by,
                reason=command.reason,
            )
            span.set_attribute("tool_group.deactivated", deactivated)

        if not deactivated:
            return self.bad_request("Tool group is already inactive")
        ...
        tool_groups_deactivated.add(1, {"has_reason": command.reason is not None})
        tool_group_processing_time.record(processing_time_ms, {"operation": "deactivate"})

        log.info(f"Deactivated tool group: {command.group_id}")
