"""Remove explicit tool from tool group command with handler."""
...
@dataclass
class RemoveExplicitToolCommand(Command[OperationResult[ToolGroupDto]]):
    """Command to remove an explicit tool from a group."""
...
class RemoveExplicitToolCommandHandler(
    CommandHandlerBase,
    CommandHandler[RemoveExplicitToolCommand, OperationResult[ToolGroupDto]],
):
    """Handler for removing explicit tools from groups."""
    ...
    async def handle_async(self, request: RemoveExplicitToolCommand) -> OperationResult[ToolGroupDto]:
        """Handle the remove explicit tool command."""
        command = request
        ...
        # Create custom span for tool removal
        with tracer.start_as_current_span("remove_explicit_tool_from_group") as span:
            ...
            span.set_attribute("tool.removed_by", removed_by or "unknown")

            # Remove tool
            removed = tool_group.remove_tool(command.tool_id, removed_by=removed_by)
        ...
        tool_group_tools_removed.add(1)
        tool_group_processing_time.record(processing_time_ms, {"operation": "remove_explicit_tool"})

        log.info(f"Removed tool '{command.tool_id}' from tool group: {command.group_id}")
