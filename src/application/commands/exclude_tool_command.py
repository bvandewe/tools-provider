"""Exclude tool from tool group command with handler."""
...
from observability import tool_group_processing_time, tool_group_tools_excluded
...
@dataclass
class ExcludeToolCommand(Command[OperationResult[ToolGroupDto]]):
    """Command to exclude a tool from a group."""

    group_id: str
    """ID of the group to modify."""

    tool_id: str
    """ID of the tool to exclude."""
...
class ExcludeToolCommandHandler(
    CommandHandlerBase,
    CommandHandler[ExcludeToolCommand, OperationResult[ToolGroupDto]],
):
    """Handler for excluding tools from groups."""
    ...
    async def handle_async(self, request: ExcludeToolCommand) -> OperationResult[ToolGroupDto]:
        """Handle the exclude tool command."""
        command = request
        ...
        add_span_attributes(
            {
                ...
                "tool.has_reason": command.reason is not None,
        ...
        if command.user_info:
            excluded_by = command.user_info.get("sub") or command.user_info.get("email")

        # Create custom span for tool exclusion
        with tracer.start_as_current_span("exclude_tool_from_group") as span:
            ...
            span.set_attribute("tool.excluded_by", excluded_by or "unknown")

            # Exclude tool
            excluded = tool_group.exclude_tool(
                command.tool_id,
                excluded_by=excluded_by,
                reason=command.reason,
            )
            span.set_attribute("tool.excluded", excluded)

        if not excluded:
            return self.bad_request(f"Tool '{command.tool_id}' is already excluded from this group")
        ...
        tool_group_tools_excluded.add(1, {"has_reason": command.reason is not None})
        tool_group_processing_time.record(processing_time_ms, {"operation": "exclude_tool"})

        log.info(f"Excluded tool '{command.tool_id}' from tool group: {command.group_id}")
