...
Following the same pattern as tools-provider.
"""
...
class CommandHandlerBase:
    ...
    def _get_tenant_id(self, user_info: dict[str, Any] | None) -> str | None:
        """Extract tenant ID from user_info dictionary.

        Checks for common tenant claim fields.
        ...
        if not user_info:
            return None
        return user_info.get("tenant_id") or user_info.get("organization_id") or user_info.get("org_id")

    async def publish_cloud_event_async(self, ev: IntegrationEvent) -> None:
        """Converts the specified event into a cloud event, then publishes it."""
        try:
