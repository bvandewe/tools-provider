"""UI controller for Knowledge Manager - serves HTML pages."""
...
class UIController(ControllerBase):
    """Controller for Knowledge Manager UI pages."""
    ...
    async def index(self, request: Request) -> FileResponse:
        """Serve the main Knowledge Manager page.
    ...
    @get("/namespaces", response_class=HTMLResponse)
    async def namespaces(self, request: Request) -> FileResponse:
        ...
        Returns:
            The namespaces.html file
        """
        namespaces_path = self.static_dir / "namespaces.html"
        return FileResponse(namespaces_path, media_type="text/html")
